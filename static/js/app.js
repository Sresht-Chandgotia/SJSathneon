...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function searchPlace(query) {
    ...
    try {
      ...
      const poi = {
        id: place.osm_id
          ? `nominatim:${place.osm_type || "x"}:${place.osm_id}`
          : `search:${Date.now()}`,
      ...
      <div style="margin-top:8px; display:flex; gap:8px; align-items:center;">
        ...
        <button data-action="fav" data-id="${escapeHtml(
          poi.id
        )}" class="chip">${favLabel}</button>
      ...
      marker.on("popupopen", (ev) => {
        ...
        if (gotoBtn)
          gotoBtn.addEventListener("click", async () => {
            try {
              if (
                window.__NAV__ &&
                typeof window.__NAV__.setDestination === "function"
              ) {
                await window.__NAV__.setDestination(
                  { lat: lat, lng: lon },
              ...
              console.warn(
                "Failed to set destination from search marker:",
                err
              );
              marker.openPopup();
            }
        ...
        const startBtn = popupEl.querySelector(
          'button[data-action="setstart"]'
        );
        if (startBtn)
          startBtn.addEventListener("click", async () => {
            try {
              if (
                window.__NAV__ &&
                typeof window.__NAV__.createStartMarker === "function"
              ) {
                await window.__NAV__.createStartMarker(
                  L.latLng(lat, lon),
                  `<strong>Start: ${escapeHtml(prettyName)}</strong>`
                );
                map.setView([lat, lon], 15);
  ...
  function addFavoriteFromPOI(poi) {
    ...
    const obj = {
      ...
      type:
        poi.type ||
        (poi.tags && (poi.tags.amenity || poi.tags.shop)) ||
        "place",
      lat: Number(poi.lat),
      lon: Number(poi.lon),
  ...
  function ensureFavNameModal() {
    if (document.getElementById("favNameModal"))
      return document.getElementById("favNameModal");
  ...
  `;
    ...
    btnSave.addEventListener("click", () => {
      ...
      modal.dispatchEvent(
        new CustomEvent("favNameSave", { detail: { name: v } })
      );
      hide();
    });
  ...
  function promptFavoriteName(defaultName = "") {
    return new Promise((resolve) => {
      ...
      const onSave = (ev) => {
        const name =
          ev.detail && ev.detail.name ? String(ev.detail.name) : null;
  ...
  async function findNearby(type, lat, lon) {
    ...
    for (let epIdx = 0; epIdx < endpoints.length; epIdx++) {
      ...
      try {
        ...
        const res = await fetchWithTimeout(
          endpoint,
          `data=${encodeURIComponent(nodeQuery)}`,
          20000
        );
      ...
      try {
        ...
        if (Array.isArray(data2.elements) && data2.elements.length > 0) {
          ...
          console.warn(
            `Overpass full query returned no elements @ ${endpoint}`
          );
        }
      } catch (err) {
    ...
    if (!responseJson || !Array.isArray(responseJson.elements)) {
      console.error(
        "Overpass/search error: All endpoints failed or returned no results."
      );
    ...
    items.forEach((it) => {
      try {
        it.distance = map.distance([lat, lon], [it.lat, it.lon]);
      } catch (e) {
        it.distance = null;
      }
    });
    ...
    showPopup(
      `Found ${items.length} ${type}(s). Showing closest ${toShow.length}.`,
      3500
    );
  }
  ...
  function renderNearbyItems(items, categoryType, centerLat, centerLon) {
    ...
    items.forEach((it) => {
      try {
        it.distance = map.distance([centerLat, centerLon], [it.lat, it.lon]);
      } catch (e) {
        it.distance = null;
      }
    });
    ...
    toShow.forEach((it, idx) => {
      const name =
        it.tags && it.tags.name
          ? it.tags.name
          : it.tags?.operator || categoryType.toUpperCase();
      const distText =
        it.distance == null
          ? ""
          : it.distance < 1000
          ? `${Math.round(it.distance)} m`
          : `${(it.distance / 1000).toFixed(2)} km`;
      const fav = isFavorite(it);
      const favLabel = fav ? "★ Favorited" : "☆ Favorite";

      const popupHtml = `<div style="min-width:170px">
      <strong>${escapeHtml(name)}</strong><br/>
      <small style="color:#888">${escapeHtml(categoryType)}${
        distText ? ` — ${distText}` : ""
      }</small>
      <div style="margin-top:8px; display:flex; gap:8px; align-items:center;">
        <button data-action="goto" data-lat="${it.lat}" data-lon="${
        it.lon
      }" class="chip">Go</button>
        <button data-action="fav" data-id="${escapeHtml(
          it.id
        )}" class="chip">${favLabel}</button>
      ...
      marker.on("popupopen", (ev) => {
        ...
        const setStartBtn = popupEl.querySelector(
          'button[data-action="setstart"]'
        );
        if (setStartBtn)
          setStartBtn.addEventListener("click", async () => {
            try {
              if (window.__NAV__?.createStartMarker) {
                await window.__NAV__.createStartMarker(
                  { lat: it.lat, lng: it.lon },
                  `<strong>Start: ${escapeHtml(name)}</strong>`
                );
          ...
          favBtn.addEventListener("click", async (e) => {
            try {
              ...
              const poi = {
                ...
                type:
                  it.type ||
                  (it.tags && (it.tags.amenity || it.tags.shop)) ||
                  "place",
                lat: it.lat,
                lon: it.lon,
                name: nameInput,
              ...
              renderFavorites();
              showPopup(
                nowFav ? "Saved to favorites" : "Updated favorite",
                1400
              );
            } catch (err) {
  ...
  (function setupAdvancedSearchRobust() {
    ...
    let searchInput =
      document.querySelector(".center-nav #search") ||
      document.querySelector("#blk-search input#search") ||
    ...
    function updateSelection() {
      ...
      children.forEach((c, i) => {
        ...
        if (i === focusedIndex)
          c.scrollIntoView({ block: "nearest", behavior: "auto" });
      });
    }
    ...
    function renderSuggestions(items) {
      ...
      items.forEach((it, idx) => {
        ...
        sub.textContent = it.address
          ? composeAddress(it.address)
          : it.display_name || "";
    ...
    function composeAddress(addrObj) {
      ...
      if (addrObj.state && parts.indexOf(addrObj.state) === -1)
    ...
    async function fetchSuggestions(query) {
      ...
      try {
        ...
        if (window.__NAV__?.userLocation) {
          liveLatLng = L.latLng(
            window.__NAV__.userLocation.lat,
            window.__NAV__.userLocation.lng
          );
        } else if (map) {
    ...
    async function chooseSuggestion(idx) {
      ...
      if (
        ...
        try {
          ...
          console.warn("setDestination failed:", err);
        }
        return;
      }
      ...
      const poi = {
        id: item.osm_id
          ? `nominatim:${item.osm_type || "x"}:${item.osm_id}`
          : `search:${Date.now()}`,
      ...
      const popupHtml = `<div style="min-width:200px">
        ...
        <div style="margin-top:8px; display:flex; gap:8px; align-items:center;">
          <button data-action="goto" data-lat="${item.lat}" data-lon="${
        item.lon
      }" class="chip">Go</button>
          <button data-action="fav" data-id="${escapeHtml(
            poi.id
          )}" class="chip">${favLabel}</button>
          <button data-action="setstart" data-lat="${item.lat}" data-lon="${
        item.lon
      }" class="chip">Set as start</button>
        </div>
      </div>`;
  ...
  if (clearDestBtn) {
    clearDestBtn.addEventListener("click", () => {
      ...
      if (
        window.__NAV__ &&
        typeof window.__NAV__.clearDestination === "function"
      ) {
        ...
        try {
          ...
          if (window.__NAV__?.destinationMarker && map) {
            if (map.hasLayer(window.__NAV__.destinationMarker))
              map.removeLayer(window.__NAV__.destinationMarker);
  ...
  if (clearStartBtn) {
    clearStartBtn.addEventListener("click", () => {
      if (
        window.__NAV__ &&
        typeof window.__NAV__.clearStartMarker === "function"
      ) {
    ...
    favs.forEach((f) => {
      ...
      el.innerHTML = `
        ...
        <div style="display:flex;gap:6px;">
          <button class="chip fav-go" data-lat="${f.lat}" data-lon="${
        f.lon
      }" style="padding:6px 8px;">Go</button>
          <button class="chip fav-remove" data-id="${escapeHtml(
            f.id
      ...
      el.querySelector(".fav-go")?.addEventListener("click", async () => {
        if (window.__NAV__?.setDestination) {
          await window.__NAV__.setDestination(
            { lat: f.lat, lng: f.lon },
            `<strong>${escapeHtml(f.name || f.type)}</strong>`
          );
  ...
  (function setupNavbarControls() {
    ...
    if (navSearchInput && panelSearchInput) {
      navSearchInput.addEventListener("input", () => {
        if (panelSearchInput.value !== navSearchInput.value)
          panelSearchInput.value = navSearchInput.value;
      });
      panelSearchInput.addEventListener("input", () => {
        if (navSearchInput.value !== panelSearchInput.value)
          navSearchInput.value = panelSearchInput.value;
      });
    }
    ...
    modePills.forEach((p) => {
      p.addEventListener("click", () => {
        ...
        if (
          window.__NAV__ &&
          typeof window.__NAV__.setRoutingMode === "function"
        ) {
    ...
    document.addEventListener("nav:close-search", () =>
      setNavSearchOpen(false)
    );
  })();
  // --------------------------------------
  ...
  document
    .getElementById("addDestinationFavBtn")
    ?.addEventListener("click", async () => {
      try {
        ...
        console.warn(e);
        showPopup("Could not save destination", 2200);
      }
    });
...
function openPanel() {
  ...
  body.classList.add("panel-open"); // 👈 triggers nav bar style changes
}
...
hudToggle.addEventListener("click", openPanel);
hudClose.addEventListener("click", closePanel);
